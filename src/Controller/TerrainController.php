...
public function backindex(TerrainRepository $terrainRepository, Request $request): Response
{
    ...
    public function newFront(Request $request, EntityManagerInterface $entityManager, UserInterface $user): Response
    {
        $terrain = new Terrain();
        $terrain->setUtilisateur($user);
        $form = $this->createForm(TerrainType::class, $terrain);
        $form->handleRequest($request);
...
#[Route('/front/mes-terrains', name: 'app_terrain_mesterrains', methods: ['GET'])]
    public function mesTerrains(TerrainRepository $terrainRepository, UserInterface $user): Response
    {
        // Récupérer les terrains de l'utilisateur connecté
        $terrains = $terrainRepository->findBy(['utilisateur' => $user]);

        return $this->render('terrain/front/mes_terrains.html.twig', [
            'terrains' => $terrains,
        ]);
    }
